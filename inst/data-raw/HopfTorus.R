...
meshHopfTorus <- function(nu, nv, nlobes, A) {
    f <- function(u, v, nlobes, A) {
    ...
    for(i in seq_len(nu)) {
      for(j in seq_len(nv)) {
          vertices[, (i-1L)*nv+j] <- f(u_[i], v_[j], nlobes, A)
      }
    }
    ...
    tmesh3d(vertices=vertices, indices=cbind(tris1, tris2))
}
