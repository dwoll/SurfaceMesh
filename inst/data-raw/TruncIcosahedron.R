...
meshTruncIcosahedron <- function(raw=TRUE, triangulate=TRUE) {
    C0 <- (1 + sqrt(5)) / 4
    C1 <- (1 + sqrt(5)) / 2
    C2 <- (5 + sqrt(5)) / 4
    C3 <- (2 + sqrt(5)) / 2
    ...
    lvf <- list("vertices"=vertices, "faces"=faces)
    if(raw) {
        lvf
    } else {
        makeMeshValid(lvf, triangulate=triangulate)
    }
}

# dataTruncIcosahedron <- meshTruncIcosahedron(raw=FALSE, triangulate=TRUE)
# wire3d(toRGL(dataTruncIcosahedron))
